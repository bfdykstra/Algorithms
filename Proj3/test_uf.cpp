...
int main (int argc, char **argv)
{
  ...
  cout << "No. of random connections to start: " << m << endl;
 ...
 // auto t_start = std::chrono::high_resolution_clock::now();
  ...
  for (int i = 0; i < num_samples; i++) {
    cs303::union_find uf(n);
    long int rand_connects = m;
    while(uf.num_components() != 1){
      auto p = u(e);
      auto q = u(e);
      uf.onion(p,q);
      rand_connects = rand_connects + 1;
    }
    //cout << "No. connections: " << rand_connects << endl;
    total += rand_connects;
  }

  auto avg_connects = total / num_samples;
 ...
 // auto t_end = std::chrono::high_resolution_clock::now();
  ...
  cout << "Average no. of random connections: " << avg_connects << endl;
